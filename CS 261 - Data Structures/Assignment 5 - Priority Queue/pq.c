...
void pq_insert(struct pq* pq, void* item, int priority) {
   ...
   */
	...
	int index_new = dynarray_size(pq->heap)-1;
	int index_parent = 0;
	
	struct pq_elem *temp_elem;
   
	while(index_parent >=0){
		index_parent = (index_new -1)/2;
		temp_elem = (struct pq_elem*)dynarray_get(pq->heap, index_parent);
		
		if(temp_elem->priority > new_elem->priority){
			dynarray_set(pq->heap, index_parent, new_elem);
			dynarray_set(pq->heap, index_new, temp_elem);
...
void* pq_first(struct pq* pq) {
   ...
   */
   ...
   //Is this really it? It seems too simple :)
...
void* pq_remove_first(struct pq* pq) {
   ...
   */
   ...
   first_elem = dynarray_get(pq->heap, 0);
   dynarray_set(pq->heap, 0, dynarray_get(pq->heap, 0));
   ...
   */
	...
	/////////////////////////////////////////////////////////////////////
	restore_heap(pq->heap, 0, 0); 
	//////NOTE!!! I added this funtion to the end of the dynarray.c file.
	/////////////////////////////////////////////////////////////////////
