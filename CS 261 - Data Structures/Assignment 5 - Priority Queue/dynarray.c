...
void restore_heap (struct dynarray* pq, int max, int priority) {
	...
	if(right < max) {
		...
		if(dynarray_get(pq, left) > dynarray_size(pq)){
			return;
		}
		else{
			small = left;
		...
		if(dynarray_get(pq, left) < dynarray_get(pq, right)){
			small = left;
		}
		else{
			small = right;
	...
	if(dynarray_get(pq, small) < dynarray_get(pq, priority)){
		
		//Swap values in array
		int temp = dynarray_get(pq, priority);
		dynarray_set(pq, priority, dynarray_get(pq, small));
		dynarray_set(pq, small, temp);
		//run again
		restore_heap(pq, max, small);
	}
}
