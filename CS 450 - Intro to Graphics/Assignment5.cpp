...
int		View;					// != 0 means to view from cockpit
...
Display( )
{
	...
	if (View==1) {
		gluLookAt(-.4, 1.8, -4.9, 1., 1., -15., 0., 1., 0.);
	}
	else {
		gluLookAt(9., 7., 11., 0., 0., 0., 0., 1., 0.);
	...
	if (useFrag == 1) {
		...
		Pattern->SetUniformVariable("uTimeF", uTimeF);
	}

	if (useVert == 1) {
		Pattern->SetUniformVariable("uTimeV", uTimeV);
	}
...
Keyboard( unsigned char c, int x, int y )
{
	...
	case 'F':
		useFrag = 1;
		useVert = 0;
		printf("frag = 1, vert = 0\n");

		break;
	case 'V':
		useVert = 1;
		useFrag = 0;
		printf("frag = 0, vert = 1\n");
